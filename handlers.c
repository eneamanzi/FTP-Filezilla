...
int handle_USER_Command(int client_socket, char *bufferIn, Session *state) {
  ...
  //salto 5 spazi (USER ) e poi ho il nome utente
...
int handle_PASS_Command(int client_socket, char *bufferIn, Session *state) {
  if (state == NULL)
    printf("null re\n");
  //dato che viene sempre effettuata la remove spaces mettere +5(come sopa) o +4 è indifferente, quello spazio tra cmd e args viene rimosso
...
int handle_LIST_Command(int client_socket, char *bufferIn, Session *state) {
  ...
  //funzionamento
    ...
    snprintf(bufferIn, BUFFER_SIZE, "150 data connection for file list.\r\n");
    send(client_socket, bufferIn, strlen(bufferIn), 0);
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      snprintf(filepath, BUFFER_SIZE*2, "%s/%s", state->current_working_dir,entry->d_name); //concatena CWD con nome del file/directory
...
int handle_DELE_Command(int client_socket, char *bufferIn, Session *state) {
  ...
  if(res == 0){
    ...
    sprintf(bufferIn, "550 Failed to delete file.\r\n");
  }
  send(client_socket, bufferIn, strlen(bufferIn), 0);
  return 0;
}
...
int handle_QUIT_Command(int client_socket, char *bufferIn, Session *state){
  snprintf(bufferIn, BUFFER_SIZE, "221 Goodbye.\r\n");
  send(client_socket, bufferIn, strlen(bufferIn), 0);
  close(client_socket);
  return 0;
}